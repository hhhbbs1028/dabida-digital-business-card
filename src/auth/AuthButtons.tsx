...
export function AuthButtons() {
  ...
  const handleSignOut = async () => {
    ...
    try {
      ...
      if (signOutError) {
        console.error('[AuthButtons] 로그아웃 오류:', signOutError);
        setError(signOutError.message);
      } else {
        console.log('[AuthButtons] 로그아웃 완료');
      }
    } catch (err: any) {
      console.error('[AuthButtons] 로그아웃 실패:', err);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <button
          ...
          disabled={!!loading}
          className="flex w-full items-center justify-center gap-3 rounded-lg border border-slate-300 bg-slate-900 px-4 py-3 text-sm font-medium text-white shadow-sm transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60"
        >
          {loading === 'apple' ? (
            <>
