...
// ============================================================================
// 텍스트 폭 추정 — 기본 위치 자동 보정에 사용
// ============================================================================
//
// 한글/한자/일본어(CJK)는 약 0.95em, 그 외(라틴·숫자·기호)는 약 0.55em 폭으로
// 가정하고 카드 폭 대비 백분율을 돌려준다. defaultPositions가 이 값을 사용해
// 긴 텍스트가 카드 좌측을 침범하지 않도록 기본 x좌표를 우측으로 보정한다.

const CJK_RE = /[　-〿぀-ヿ㄰-㆏㐀-䶿一-鿿가-힯＀-￯]/;

function charEmFactor(ch: string): number {
  return CJK_RE.test(ch) ? 0.95 : 0.55;
}

/**
 * 텍스트가 카드 폭에서 차지할 비율(%)을 추정한다.
 * - fontRem: cardElements가 textStyle/linkStyle에 넘기는 base/baseP rem 값.
 *            renderHelpers의 환산식(rem→px→cqw)과 동일 단위계로 맞춘다.
 * - withIcon: 링크형 요소(아이콘 + gap 포함) 여부.
 */
export function estimateTextWidthPct(
  text: string,
  fontRem: number,
  isPortrait: boolean,
  withIcon = false,
  ...
  if (!text) return 0;
  const cardWidth = isPortrait ? BASE_WIDTH_PORTRAIT_PX : BASE_WIDTH_LANDSCAPE_PX;
  const fontPx = fontRem * REM_PX;
  const fontPct = (fontPx / cardWidth) * 100;
  let units = 0;
  for (const ch of text) units += charEmFactor(ch);
  let widthPct = fontPct * units;
  if (withIcon) widthPct += ((ICON_PX + LINK_GAP_PX) / cardWidth) * 100;
  return widthPct;
}
